...
use app\admin\model\WapModel;
use app\admin\model\WapNavModel;
...
class Wap extends Base
{
    ...
    public function save_config(){
        if(request()->isAjax()){
            ...
            $file  = request()->file('wap_site_logo');
            if($file || (isset($param['delLogo']) && $param['delLogo'] == 1)){
                $config['wap_site_logo'] = $upload->uploadImage($file);
                $logo = Db::name('config')->where('name','wap_site_logo')->value('value');
            }

            $file_code  = request()->file('wap_site_qrcode');
            if($file_code || (isset($param['delQrcode']) && $param['delQrcode'] == 1)){
                $config['wap_site_qrcode'] = $upload->uploadImage($file_code);
                $qrcode = Db::name('config')->where('name','wap_site_qrcode')->value('value');
            }
    ...
    public function add_nav()
    {
        ...
        $sortnum = Db::name('wap_nav')->max('sortnum') + 10;
        $cateModel = new ArticleCateModel();
        $map['parent_id'] = 0;
    ...
    public function edit_nav()
    {
        ...
        if(request()->isAjax()){
            ...
            if($file || (isset($param['delPhoto']) && $param['delPhoto'] == 1)){
                $upload = new Upload();
                $param['photo'] = $upload->uploadImage($file);
                $photo = Db::name('wap_nav')->where('id',$param['id'])->value('photo');
            }else{
                unset($param['photo']);
            }
    ...
    public function del_nav()
    {
        ...
        $photo = Db::name('wap_nav')->where('id',$id)->value('photo');
