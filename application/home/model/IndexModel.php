...
class IndexModel extends Model
{
     ...
     * [获取模块列表]
     */
    public function getStructureList($map)
    {
        return Db::name('structure')->where($map)->order('sortnum asc')->select();
    }

    /**
     * [ 获取子版块列表]
     */
    public function getModuleList($map)
    {
        return Db::name('structure_module')->where($map)->order('sortnum asc')->select();
    }
    ...
    public function getArticleList($map, $limits=null, $type=0)
    {
        ...
        if($type == 1){
            ...
            $result['url'] = getCateUrl($result['cate_id'],$result['website'],$result['catedir']);
        }else{
