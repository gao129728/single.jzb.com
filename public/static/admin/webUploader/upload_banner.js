		...
		uploader.on('uploadAccept', function(obj, response ) {
			...
			if(response.code == 0){
                ...
                var html = '<li><input type="hidden" class="img_src" name="img_src['+len+']" value="'+ response.data +'">'+
                    ...
                    '<div class="col-sm-6"><input type="text" name="img_name['+len+']" value="'+name+'" placeholder="请输入标题" class="form-control"></div>'+
                    '<label class="col-sm-2 control-label">状态</label>'+
                    '<div class="col-sm-3"><input type="checkbox" name="state['+ len +']" value="1" class="lcs_check" lcs-text="显示|隐藏" autocomplete="off" checked/></div>'+
                    '</div><div class="form-group">'+
                    '<label class="col-sm-2 control-label">链接</label>'+
                    '<div class="col-sm-10"><input type="text" name="img_url['+len+']" placeholder="请输入链接地址" class="form-control"></div>'+
                    '</div></div></div>'+
                ...
                $('#multi-img-sortable .lcs_check').lc_switch();
