	...
	$.fn.lc_switch = function(on_text, off_text) {
		...
		return this.each(function(){
			...
			if( !$(this).parent().hasClass('lcs_wrap') ) {
				...
				// default texts
				var lcs_text = $(this).attr('lcs-text');
				var ckd_on_txt =  '';
				var ckd_off_txt = '';
				if(typeof(lcs_text) != 'undefined' && lcs_text != ''){
					var lcs_text_arr = lcs_text.split("|");
					ckd_on_txt = lcs_text_arr[0];
					ckd_off_txt = lcs_text_arr[1];
				}
			   ...
			   // labels structure
				...
				if( $(this).is(':input') && ($(this).attr('type') == 'checkbox' || $(this).attr('type') == 'radio') ) {
					if($(this).hasClass('lcs_sm')){
						$(this).wrap('<div class="lcs_wrap lcs_sm"></div>');
					}else{
