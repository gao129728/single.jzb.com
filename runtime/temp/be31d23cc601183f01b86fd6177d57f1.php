<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:83:"E:\php_project\single.jzb.com\public/../application/admin/view/apply/job_index.html";i:1532138656;s:81:"E:\php_project\single.jzb.com\public/../application/admin/view/public/header.html";i:1532138656;s:81:"E:\php_project\single.jzb.com\public/../application/admin/view/public/footer.html";i:1532138656;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="wrapper wrapper-content animated fadeInRight">
    ...
    <div class="ibox float-e-margins">
        ...
        <div class="ibox-content">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <form name="admin_list_sea" class="form-search" method="get" action="<?php echo url('job_index'); ?>">
                        ...
                        <script id="list-template" type="text/html">
                            {{# for(var i=0;i<d.length;i++){  }}
                                <tr class="long-td">
                                    ...
                                    <td class="edit-td"><div class="updateText" data-check="number" onclick="toggleInput(this,{{d[i].id}},'sortnum','job')">{{d[i].sortnum}}</div></td>
                                    <td>{{d[i].name}}</td>
                                    <td>{{d[i].phone}}</td>
                                    <td>{{d[i].cate_name}}</td>
                                    <td>{{d[i].sex}}</td>
                                    <td>{{d[i].age? d[i].age+'岁':''}}</td>
                                    <td>{{d[i].college}}</td>
                                    <td>{{d[i].graduate_time}}</td>
                                    ...
                                    <td>
                                        <a href="javascript:;" onclick="view_job({{d[i].id}},{{d[i].page}})" class="btn btn-blue btn-xs btn-outline">
                                            <i class="fa fa-eye"></i> 查看</a>&nbsp;&nbsp;
                                        <a href="javascript:;" onclick="del_job({{d[i].id}},{{d[i].page}})" class="btn btn-danger btn-xs btn-outline">
                                            <i class="fa fa-trash-o"></i> 删除</a>
                                    </td>
                                </tr>
                            {{# } }}
                            <tr>
                               <td colspan="12" class="handle-tr">
...
<script type="text/javascript">
    ...
    function batch_delete(){
        ...
        layer.confirm('确认删除所选记录吗?', {icon: 3, title:'提示'}, function(index){
            ids = ids.substring(1);
            $.getJSON('<?php echo url("apply/all_del_job"); ?>', {'ids' : ids}, function(res){
                if(res.code == 1){
    ...
    function Ajaxpage(curr){
        var name=$('#name').val();
        var phone = $('#phone').val();
        $.getJSON('<?php echo url("apply/job_index"); ?>', {
...
//查看
function view_job(id, page){
    var name=$('#name').val();
    var phone = $('#phone').val();
    location.href = '<?php echo url("apply/view_job"); ?>?id='+id+'&name='+name+'&phone='+phone+'&cur_page='+page;
}

//删除
function del_job(id, page){
    layer.confirm('确认删除此记录?', {icon: 3, title:'提示'}, function(index){
        $.getJSON('<?php echo url("del_job"); ?>', {'id':id}, function(res){
            if(res.code == 1){
                layer.msg(res.msg,{icon:1,time:1500,shade: 0.1},function(){
                    refresh_page(page);
                });
...
function refresh_page(page){
    var name=$('#name').val();
    var phone = $('#phone').val();
    location.href = '<?php echo url("apply/job_index"); ?>?name='+name+'&phone='+phone+'&cur_page='+page;
}
