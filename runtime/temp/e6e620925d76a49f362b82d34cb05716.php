<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:81:"E:\php_project\single.jzb.com\public/../application/admin/view/apply/message.html";i:1532138656;s:81:"E:\php_project\single.jzb.com\public/../application/admin/view/public/header.html";i:1532138656;s:81:"E:\php_project\single.jzb.com\public/../application/admin/view/public/footer.html";i:1532138656;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="wrapper wrapper-content animated fadeInRight">
    ...
    <div class="ibox float-e-margins">
        ...
        <div class="ibox-content">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <form name="admin_list_sea" class="form-search" method="get" action="<?php echo url('message'); ?>">
                        ...
                        <script id="list-template" type="text/html">
                            {{# for(var i=0;i<d.length;i++){  }}
                                <tr class="long-td">
                                    ...
                                    <td class="edit-td"><div class="updateText" data-check="number" onclick="toggleInput(this,{{d[i].id}},'sortnum','message')">{{d[i].sortnum}}</div></td>
                                    <td>{{d[i].name}}</td>
                                    <td>{{d[i].phone}}</td>
                                    <td>{{d[i].email}}</td>
                                    <td>{{d[i].content}}</td>
                                    ...
                                    <td>
                                        <a href="javascript:;" onclick="view_message({{d[i].id}},{{d[i].page}})" class="btn btn-blue btn-xs btn-outline">
...
<script type="text/javascript">
    ...
    function batch_delete(){
        ...
        layer.confirm('确认删除所选记录吗?', {icon: 3, title:'提示'}, function(index){
            ids = ids.substring(1);
            $.getJSON('<?php echo url("apply/all_del_message"); ?>', {'ids' : ids}, function(res){
                if(res.code == 1){
    ...
    function Ajaxpage(curr){
        var name=$('#name').val();
        var phone = $('#phone').val();
        $.getJSON('<?php echo url("apply/message"); ?>', {
...
function view_message(id, page){
    var name=$('#name').val();
    var phone = $('#phone').val();
    location.href = '<?php echo url("apply/view_message"); ?>?id='+id+'&name='+name+'&phone='+phone+'&cur_page='+page;
}

//删除文章
function del_message(id, page){
    layer.confirm('确认删除此留言?', {icon: 3, title:'提示'}, function(index){
        $.getJSON('<?php echo url("del_message"); ?>', {'id':id}, function(res){
            if(res.code == 1){
                layer.msg(res.msg,{icon:1,time:1500,shade: 0.1},function(){
                    refresh_page(page);
                });
...
function refresh_page(page){
    var name=$('#name').val();
    var phone = $('#phone').val();
    location.href = '<?php echo url("apply/message"); ?>?name='+name+'&phone='+phone+'&cur_page='+page;
}
