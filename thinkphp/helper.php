...
if (!function_exists('cookieCrypt')){
    /**
     * Cookie加密
     * @param string|array  $name cookie名称，如果为数组表示进行cookie设置
     * @param string         $value cookie值 字符串
     * @param mixed         $option 参数
     * @return mixed
     */
    function cookieCrypt($name, $value = '', $option = null)
    {
        $key = '&d$c^s';  //加密key
        $value = (string)$value;
        ...
        if (is_array($name)) {
            ...
            // 获取
            $val = 0 === strpos($name, '?') ? Cookie::has(substr($name, 1), $option) : Cookie::get($name, $option);

            if(!empty($key)){
                $key=md5($key);
                $sc = new \think\SysCrypt($key);
                $val=$sc->php_decrypt($val);
            }
